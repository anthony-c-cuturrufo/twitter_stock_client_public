...
(* converts json [j] into a [info] type *)
let info_of_json j =
  {
...
(* converts json string [json] and the given stock [ticker] in an [info]
   type *)
...
(* checks volume of [ticker] in its corresponding [json] to see if it is
   a valid price from config*)
